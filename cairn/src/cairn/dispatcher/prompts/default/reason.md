...
- Use `code_index.audit_context.entrypoint_summaries`, `business_flow_traces`, `file_slices`, and `planner_focus_packs` to choose concrete audit directions instead of broad repository-wide guesses. These are relevance-selected views of the complete server-side index; omitted index rows remain available to focused Explore tasks through source reads.
...
- Plan from `business_graph` semantic nodes first: roles, business functions, resources, states, controls, trust boundaries, and external systems. Use audit nodes as hypotheses and expand evidence-layer neighbors only to locate the backing entrypoints, handlers, data objects, and calls. The omitted evidence graph remains available through focused source reads and coverage queues.
...
- Each intent may name at most 5 source files and at most 8 closely related candidate IDs. Split larger module families into separate, non-overlapping intents so all three general audit workers can make progress concurrently.
- Prefer one `planner_focus_pack` or one coherent entrypoint family per intent. Do not create a single intent covering a numbered lesson range, an entire controller directory, or more than one independently reviewable business workflow.
